 ...
 * 和作为千进制货币符显示；用货币符号代替；如果双写，用国际货币符号代替。如果出现在一个模式中，用货币十进制分隔符代替十进制分隔符。 X 前缀或后缀中使用的任何其它字符，用来引用前缀或后缀中的特殊字符。
 *
 * @author harry
 */
...
public class DecimalUtility {
    ...
    public static String toChineseString(BigDecimal num) {
        ...
        strNumbers = new DecimalFormat("#0").format(num.multiply(new BigDecimal("100"))); //将num乘100并转换成字符串形式
